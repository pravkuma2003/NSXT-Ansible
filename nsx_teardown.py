...
class NSXClient(object):
    ...
    def __init__(self, host, username, password, nsx_cert, key,
                 ca_cert, cluster, remove, t0_uuid, t1_uuid, all_res):
        self.host = host
    ...
    def make_delete_call(self, full_url):
        if self.use_cert:
            ...
            return requests.delete(full_url, auth=(self.username,
                                                                self.password),
                                   headers=self.header,
                                   verify=False)
        ...
        for ls in lswitches:
            ...
            try:
                snat_rules = self.get_secondary_resource(
                    'LogicalRouter', self._top_tier_router['id'], 'nat/rules')
                ...
                print("ERROR: Failed to unconfigure nat rule for %s "
                      "from top-tier router: %s" % (subnet, e))
            else:
                print("Successfully unconfigured nat rule for %s "
                      "from top-tier router" % subnet)
    ...
    def cleanup_ncp_tier0_logical_ports(self):
        ...
        """
        if self._top_tier_router['router_type'] != 'TIER0':
            return
        ...
        for t1 in tier1_routers:
            print("Router link port from %s to %s to be removed" %
                  (self._top_tier_router['display_name'], t1['display_name']))
            try:
                self.remove_router_link_port(t1['id'])
            except Exception as e:
                print("Error removing router link port from %s to %s" %
                      (self._top_tier_router['display_name'],
                       t1['display_name']), e)
            else:
                if not self._remove:
                    continue
    ...
    def authenticate(self):
        ...
        full_url = self.host
        if self.use_cert:
            response = requests.get(full_url, cert=(self.nsx_cert,
                                self.key),
                                headers=self.header,
                                verify=False)
        else:
...
def validate_options(options):
    ...
    if (not options.password and not options.username and
        ...
        sys.exit(1)
    if options.t0_uuid and options.t1_uuid:
        print("Tier-0 and Tier-1 routers can not be both specified")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_option("--t0-uuid", dest="t0_uuid",
                      help="Specify the tier-0 router uuid. Must be "
                           ...
                           "cluster tag")
    parser.add_option("--t1-uuid", dest="t1_uuid",
                      help="Specify the tier-1 router uuid. Must be "
                           "specified for a single-tier1 topology")
